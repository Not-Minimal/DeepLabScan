...
class RoboflowDataLoader:
    ...
    def download_dataset(
        self, location: str = "./data/raw", format: str = "yolov8"
    ) -> str:
        """
        ...
        self.dataset = self.project_obj.version(self.version).download(
            model_format=format, location=location
        )

        print(f"Dataset descargado en: {self.dataset.location}")

        # Log debug: listar archivos descargados
        logger.debug("Listado de archivos en %s:", self.dataset.location)
        try:
            for root, dirs, files in os.walk(self.dataset.location):
                for fname in files:
                    file_path = os.path.join(root, fname)
                    rel = os.path.relpath(file_path, self.dataset.location)
                    logger.debug(" - %s", rel)
        except Exception as e:
            logger.debug("Error al listar archivos descargados: %s", e)
